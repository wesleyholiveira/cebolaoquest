...
module.exports = (tableName, models) => {
    ...
    const duplicatedValues = columns.map(column => `${column} = new.${column}`).join(',')

    let query = `INSERT INTO \`${tableName}\` (${columns.join(',')}) VALUES ${values.slice(0, -1)} as new ON DUPLICATE KEY UPDATE ${duplicatedValues}`;
    return query
}